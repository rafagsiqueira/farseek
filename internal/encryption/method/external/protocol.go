...
const Magic = "Farseek-External-Encryption-Method"

// Header is the initial message the external method writes to stdout as a single-line JSON.
type Header struct {
	// Magic must always be "Farseek-External-Encryption-Method"
	Magic string `json:"magic"`
	// Version must always be 1.
	Version int `json:"version"`
}

// InputV1 is an encryption/decryption request from Farseek to the external method. Farseek writes this message
