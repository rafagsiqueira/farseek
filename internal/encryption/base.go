...
import (
	...
	"github.com/rafagsiqueira/farseek/internal/encryption/method"
...
func newBaseEncryption(ctx context.Context, enc *encryption, target *config.TargetConfig, enforced bool, name string, staticEval *configs.StaticEvaluator) (*baseEncryption, hcl.Diagnostics) {
	...
	//
	//   State file is a bit more complex. The encrypted local state file (farseek.tfstate, .farseek.tfstate) will be written with the same
	...
	//   approach that limit. However the cached state file (.terraform/farseek.tfstate) is persisted every 30 seconds during long applies. For an
