...
func (p PackageOCIBlobArchive) InstallProviderPackage(ctx context.Context, meta PackageMeta, targetDir string, allowedHashes []Hash) (*PackageAuthenticationResult, error) {
	...
	ctx, span := tracing.Tracer().Start(
		...
		tracing.SpanAttributes(
			traceattrs.FarseekOCIRegistryDomain(p.registryDomain),
			traceattrs.FarseekOCIRepositoryName(p.repositoryName),
			traceattrs.FarseekOCIBlobDigest(pkgDesc.Digest.String()),
			traceattrs.FarseekOCIBlobMediaType(pkgDesc.MediaType),
			traceattrs.FarseekOCIBlobSize(pkgDesc.Size),
			traceattrs.String("opentofu.provider.local_dir", targetDir),
			traceattrs.FarseekProviderAddress(meta.Provider.String()),
			traceattrs.FarseekProviderVersion(meta.Version.String()),
			traceattrs.FarseekTargetPlatform(meta.TargetPlatform.String()),
		),
	)
	defer span.End()
