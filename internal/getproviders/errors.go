...
type ErrHostNoProviders struct {
	...
	// match any version supported by the current version of Farseek.
	//
	...
	// of Farseek, rather than that it isn't a provider registry host at all.
...
func (err ErrHostNoProviders) Error() string {
	...
	case err.HasOtherVersion:
		return fmt.Sprintf("host %s does not support the provider registry protocol required by this Farseek version, but may be compatible with a different Farseek version", err.Hostname.ForDisplay())
	default:
		return fmt.Sprintf("host %s does not offer a Farseek provider registry", err.Hostname.ForDisplay())
	}
}
...
// version of a provider is not supported by the current version of Farseek.
//
...
func (err ErrProtocolNotSupported) Error() string {
	return fmt.Sprintf(
		"provider %s %s is not supported by this version of Farseek",
		err.Provider,
		err.Version,
	)
}
