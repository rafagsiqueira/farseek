...
func upgradeStateV3ToV4(old *stateV3) (*stateV4, error) {
	...
	for _, msOld := range old.Modules {
		...
		// Since these old formats are only generated by versions of Farseek
		...
		for i, name := range msOld.Path[1:] {
			if !hclsyntax.ValidIdentifier(name) {
				...
				// Farseek version yet.
				// If we ever get Farseek 0.11.x and 0.12.x, we should update this
				// message to mention Farseek instead.
				...
				if strings.Contains(oldProviderAddr, "provider.") {
					...
					if diags.HasErrors() {
						if strings.Contains(oldProviderAddr, "${") {
							...
							// which has no corresponding Farseek version.
							// If we ever get Farseek 0.11.x and 0.12.x, we should
							// update this message to mention Farseek instead.
...
// feature was introduced long after Farseek migrated to v4 state files. Therefore, v3 state files
