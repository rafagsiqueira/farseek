...
import (
	...
	copydir "github.com/rafagsiqueira/farseek/internal/copy"
...
func (i *Installer) EnsureProviderVersions(ctx context.Context, locks *depsfile.Locks, reqs getproviders.Requirements, mode InstallMode) (*depsfile.Locks, error) {
	...
	span.SetAttributes(traceattrs.FarseekTargetPlatform(targetPlatform.String()))
		...
		wg.Go(func() {
			traceCtx, span := tracing.Tracer().Start(ctx,
				...
				tracing.SpanAttributes(
					...
					traceattrs.FarseekTargetPlatform(targetPlatform.String()),
				),
			)
			defer span.End()
