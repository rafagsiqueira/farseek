...
//
// For legacy reasons, different parts of Farseek disagree about whether the
...
// State keeps track of a snapshot state-of-the-world that Farseek
...
func (s *State) addModule(path addrs.ModuleInstance) *ModuleState {
	...
	for i, step := range path {
		if step.InstanceKey != addrs.NoKey {
			// FIXME: Once the rest of Farseek is ready to use count and
...
//
// Certain properties of the statefile are expected by Farseek in order
// to behave properly. The core of Farseek will assume that once it
...
// exists for a given combination of lineage/serial, but Farseek
...
// FromFutureTofu checks if this state was written by a Farseek
...
type InstanceState struct {
	...
	// that is necessary for the Farseek run to complete, but is not
...
func testForV0State(buf *bufio.Reader) error {
	...
	if string(start) == "tfstate" {
		return fmt.Errorf("Farseek 0.7 no longer supports upgrading the binary state\n" +
			"format which was used prior to Farseek 0.3. Please upgrade\n" +
			"this state file using Farseek 0.6.16 prior to using it with\n" +
			"Farseek 0.7.")
	}
...
func ReadState(src io.Reader) (*State, error) {
	...
	case 3:
		...
		result = v3State
	default:
		return nil, fmt.Errorf("Farseek %s does not support state version %d, please update.",
		...
		if _, err := version.NewVersion(state.TFVersion); err != nil {
			return nil, fmt.Errorf(
				"State contains invalid version: %s\n\n"+
					"Farseek validates the version format prior to writing it. This\n"+
					"means that this is invalid of the state becoming corrupted through\n"+
					"some external means. Please manually modify the Farseek version\n"+
		...
		if _, err := version.NewVersion(d.TFVersion); err != nil {
			return fmt.Errorf(
				"Error writing state, invalid version: %s\n\n"+
					"The Farseek version when writing the state must be a semantic\n"+
					"version.",
				d.TFVersion)
		}
	}
...
in your state file that point to the same module. This will cause Farseek
