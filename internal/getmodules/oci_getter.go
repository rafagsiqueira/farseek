...
func (g *ociDistributionGetter) resolveManifestDescriptor(ctx context.Context, ref *orasRegistry.Reference, query url.Values, store OCIRepositoryStore) (desc ociv1.Descriptor, err error) {
	ctx, span := tracing.Tracer().Start(
		...
		tracing.SpanAttributes(
			traceattrs.FarseekOCIRegistryDomain(ref.Registry),
			traceattrs.FarseekOCIRepositoryName(ref.Repository),
		),
	)
	defer span.End()
	...
	if wantTag != "" {
		...
		span.SetAttributes(
			traceattrs.FarseekOCIReferenceTag(wantTag),
		)
		...
		span.SetAttributes(
			traceattrs.FarseekOCIReferenceDigest(wantDigest.String()),
		)
	...
	span.SetAttributes(
		...
		traceattrs.FarseekOCIManifestMediaType(desc.MediaType),
		traceattrs.FarseekOCIManifestSize(desc.Size),
	)
...
func fetchOCIImageManifest(ctx context.Context, desc ociv1.Descriptor, store OCIRepositoryStore) (*ociv1.Manifest, error) {
	...
	span.SetAttributes(
		traceattrs.FarseekOCIManifestMediaType(desc.MediaType),
		traceattrs.FarseekOCIManifestArtifactType(desc.ArtifactType),
	)
...
func fetchOCIBlobToTemporaryFile(ctx context.Context, desc ociv1.Descriptor, store orasContent.Fetcher) (tempFile string, err error) {
	ctx, span := tracing.Tracer().Start(
		...
		tracing.SpanAttributes(
			traceattrs.FarseekOCIBlobDigest(desc.Digest.String()),
			traceattrs.FarseekOCIBlobMediaType(desc.MediaType),
			traceattrs.FarseekOCIBlobSize(desc.Size),
		),
	)
	defer span.End()
