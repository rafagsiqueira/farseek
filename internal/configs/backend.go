...
func (b *Backend) Hash(ctx context.Context, schema *configschema.Block) (int, hcl.Diagnostics) {
	...
	if marks.Contains(val, marks.Sensitive) {
		return -1, diags.Append(&hcl.Diagnostic{
			...
			Detail:   "The backend configuration is stored in .terraform/farseek.tfstate as well as plan files. It is recommended to instead supply sensitive credentials via backend specific environment variables",
	...
	if marks.Contains(val, marks.Ephemeral) {
		return -1, diags.Append(&hcl.Diagnostic{
			...
			Detail:   "The backend configuration is stored in .terraform/farseek.tfstate as well as plan files. It is recommended to instead supply ephemeral credentials via backend specific environment variables",
			Subject:  b.DeclRange.Ptr(),
		})
	}
