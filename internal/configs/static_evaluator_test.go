...
terraform {
	backend "sensitive" {
		...
		diags: []string{`eval.tf:6,2-21: Backend config contains sensitive values; The backend configuration is stored in .terraform/farseek.tfstate as well as plan files. It is recommended to instead supply sensitive credentials via backend specific environment variables`},
	}, {
		ident: "ephemeral",
		body: `
...
terraform {
	backend "my_backend" {
		...
		diags: []string{`eval.tf:7,2-22: Backend config contains ephemeral values; The backend configuration is stored in .terraform/farseek.tfstate as well as plan files. It is recommended to instead supply ephemeral credentials via backend specific environment variables`},
	}}
