...
// LoadConfigFileFromSource parses the given source code as a config file.
func (p *Parser) LoadConfigFileFromSource(src []byte, filename string) (*File, hcl.Diagnostics) {
	var file *hcl.File
	var diags hcl.Diagnostics
	switch {
	case strings.HasSuffix(filename, ".json"):
		file, diags = p.p.ParseJSON(src, filename)
	default:
	...
	ret, moreDiags := loadConfigFileBody(file.Body, filename, false, p.allowExperiments)
