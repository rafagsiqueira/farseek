...
func TestProviderTampering(t *testing.T) {
	...
	pluginDir := filepath.Join(".farseek", "providers", "registry.opentofu.org", "hashicorp", "null", providerVersion, getproviders.CurrentPlatform.String())
	...
	t.Run("lock file modified before plan", func(t *testing.T) {
		...
		err := os.WriteFile(filepath.Join(workDir, ".farseek.lock.hcl"), []byte(``), 0600)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("lock file modified after plan", func(t *testing.T) {
		...
		err = os.Remove(filepath.Join(workDir, ".farseek.lock.hcl"))
		if err != nil {
			t.Fatal(err)
		}
...
func TestProviderLocksFromPredecessorProject(t *testing.T) {
	...
	locks, diags := depsfile.LoadLocksFromBytes(newLocks, ".farseek.lock.hcl")
	if diags.HasErrors() {
