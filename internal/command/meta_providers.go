...
import (
	...
	terraformProvider "github.com/rafagsiqueira/farseek/internal/builtin/providers/tf"
	"github.com/rafagsiqueira/farseek/internal/getproviders"
	"github.com/rafagsiqueira/farseek/internal/logging"
	tfplugin "github.com/rafagsiqueira/farseek/internal/plugin"
	tfplugin6 "github.com/rafagsiqueira/farseek/internal/plugin6"
...
func (m *Meta) providerDevOverrideInitWarnings() tfdiags.Diagnostics {
	...
	detailMsg.WriteString("\nSkip farseek init when using provider development overrides. It is not necessary and may error unexpectedly.")
...
func (m *Meta) providerFactories() (map[addrs.Provider]providers.Factory, error) {
	...
	// call "unmanaged" because Farseek isn't responsible for starting
	...
	// and orchestrating one or more non-interactive Farseek runs that then
	// exercise it.
	...
	for provider, lock := range providerLocks {
		...
		checkProvider := func() error {
			...
			if allowedHashes := lock.PreferredHashes(); len(allowedHashes) != 0 {
				...
				if !matched {
					return fmt.Errorf(
						"the cached package for %s %s (in %s) does not match any of the checksums recorded in the dependency lock file, run farseek init to ensure all providers are correctly installed",
						provider, version, cacheDir.BasePath(),
					)
				}
			}
