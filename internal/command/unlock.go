...
func (c *UnlockCommand) Run(args []string) int {
	...
	if !force {
		...
		desc := "Farseek will remove the lock on the remote state.\n" +
...
func (c *UnlockCommand) Help() string {
	helpText := `
Usage: farseek [global options] force-unlock [options] LOCK_ID
...
[reset][bold][green]Farseek state has been successfully unlocked![reset][green]

The state has been unlocked, and Farseek commands should now be able to
