...
func (c *VersionCommand) Help() string {
	helpText := `
Usage: farseek [global options] version [options]

  Displays the version of Farseek and all installed plugins
...
func (c *VersionCommand) Run(args []string) int {
	...
	fmt.Fprintf(&versionString, "Farseek v%s", c.Version)
	...
	// empty or incomplete if the user hasn't successfully run "farseek init"
	// since the most recent change to dependencies.
	//
	// Generally-speaking this is a best-effort thing that will give us a good
	// result in the usual case where the user successfully ran "farseek init"
...
func (c *VersionCommand) Synopsis() string {
	return "Show the current Farseek version"
}
