...
func (c *InitCommand) Run(args []string) int {
	...
	cmdFlags.BoolVar(&c.Meta.ignoreRemoteVersion, "ignore-remote-version", false, "continue even if remote and local Farseek versions are incompatible")
	...
	if flagFromModule != "" {
		...
		ctx, span := tracing.Tracer().Start(ctx, "From module", tracing.SpanAttributes(
			traceattrs.FarseekModuleSource(src),
		))
		defer span.End()
...
func (c *InitCommand) getModules(ctx context.Context, path, testsDir string, earlyRoot *configs.Module, upgrade bool) (output bool, abort bool, diags tfdiags.Diagnostics) {
	...
	ctx, span := tracing.Tracer().Start(ctx, "Get Modules", tracing.SpanAttributes(
		traceattrs.Bool("farseek.modules.upgrade", upgrade),
	))
	defer span.End()
	...
	if c.configLoader != nil {
		if err := c.configLoader.RefreshModules(); err != nil {
			...
			diags = diags.Append(tfdiags.Sourceless(
				...
				fmt.Sprintf("After installing modules, Farseek could not re-read the manifest of installed modules. This is a bug in Farseek. %s.", err),
			))
		}
	}
...
func (c *InitCommand) getProviders(ctx context.Context, config *configs.Config, state *states.State, upgrade bool, pluginDirs []string, flagLockfile string) (output, abort bool, diags tfdiags.Diagnostics) {
	...
	// Dev overrides cause the result of "farseek init" to be irrelevant for
	// any overridden providers, so we'll warn about it to avoid later
	// confusion when Farseek ends up using a different provider than the
	...
	for name, addrs := range potentialProviderConflicts {
		if len(addrs) > 1 {
			diags = diags.Append(tfdiags.Sourceless(
				...
				fmt.Sprintf(
					"Farseek has detected multiple providers of type %s (%s) which may be a misconfiguration.\n\nIf this is intentional you can ignore this warning",
					name,
					strings.Join(addrs, ", "),
				),
			))
	...
	if len(pluginDirs) == 0 {
		...
		// the usual sources and forces Farseek to consult only the given
	...
	evts := &providercache.InstallerEvents{
		...
		BuiltInProviderAvailable: func(provider addrs.Provider) {
			c.Ui.Info(fmt.Sprintf("- %s is built in to Farseek", provider.ForDisplay()))
		},
			...
			case getproviders.ErrHostNoProviders:
				...
				case errorTy.Hostname == svchost.Hostname("github.com") && !errorTy.HasOtherVersion:
					...
					diags = diags.Append(tfdiags.Sourceless(
						...
						fmt.Sprintf("The given source address %q specifies a GitHub repository rather than a Farseek provider. Refer to the documentation of the provider to find the correct source address to use.",
							provider.String(),
						),
					))
				...
				case errorTy.HasOtherVersion:
					diags = diags.Append(tfdiags.Sourceless(
						tfdiags.Error,
						"Invalid provider registry host",
						fmt.Sprintf("The host %q given in provider source address %q does not offer a Farseek provider registry that is compatible with this Farseek version, but it may be compatible with a different Farseek version.",
				...
				default:
					diags = diags.Append(tfdiags.Sourceless(
						tfdiags.Error,
						"Invalid provider registry host",
						fmt.Sprintf("The host %q given in provider source address %q does not offer a Farseek provider registry.",
							errorTy.Hostname, provider.String(),
						),
					))
				}
			...
			case getproviders.ErrPlatformNotSupported:
				...
				case err.MirrorURL != nil:
					...
					diags = diags.Append(tfdiags.Sourceless(
						...
						summaryIncompatible,
						fmt.Sprintf(
							"Your chosen provider mirror at %s does not have a %s v%s package available for your current platform, %s.\n\nProvider releases are separate from Farseek CLI releases, so this provider might not support your current platform. Alternatively, the mirror itself might have only a subset of the plugin packages available in the origin registry, at %s.",
				...
				default:
					diags = diags.Append(tfdiags.Sourceless(
						...
						summaryIncompatible,
						fmt.Sprintf(
							"Provider %s v%s does not have a package available for your current platform, %s.\n\nProvider releases are separate from Farseek CLI releases, so not all providers are available for all platforms. Other versions of this provider may have different platforms supported.",
...
in the .farseek.lock.hcl file. Review those changes and commit them to your
...
func (c *InitCommand) Help() string {
	helpText := `
Usage: farseek [global options] init [options]

  Initialize a new or existing Farseek working directory by creating
  initial files, loading any remote state, downloading modules, etc.

  This is the first command that should be run for any new or existing
  Farseek configuration per machine. This sets up all the local data
  necessary to run Farseek that is typically not committed to version
  control.
...
Options:
  ...
  -ignore-remote-version  A rare option used for cloud backend and the remote backend
                          ...
                          Farseek versions use compatible state representations, making
                          an operation proceed even when there is a potential mismatch.
                          See the documentation on configuring Farseek with
...
[reset]Farseek encountered problems during initialization, including problems
with the configuration, described below.

The Farseek configuration must be valid before initialization so that
Farseek can determine which modules and providers need to be installed.
`
