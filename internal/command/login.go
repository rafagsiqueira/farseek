...
import (
	...
	"github.com/rafagsiqueira/farseek/internal/command/cliconfig"
...
func (c *LoginCommand) Run(args []string) int {
	...
	case *disco.ErrVersionNotSupported:
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Warning,
			"Host does not support Farseek login",
			fmt.Sprintf("The given hostname %q allows creating Farseek authorization tokens, but requires a newer version of Farseek CLI to do so.", dispHostname),
		))
	default:
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Warning,
	...
	if clientConfig == nil {
		...
		case *disco.ErrServiceNotProvided:
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				"Host does not support Farseek tokens API",
				fmt.Sprintf("The given hostname %q does not support creating Farseek authorization tokens.", dispHostname),
			))
		case *disco.ErrVersionNotSupported:
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
	...
	// Prefer Farseek login if available
	if clientConfig != nil {
		...
		default:
			tokenDiags = tokenDiags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				"Host does not support Farseek login",
				fmt.Sprintf("The given hostname %q does not allow any OAuth grant types that are supported by this version of Farseek.", dispHostname),
			))
		}
		if oauthToken != nil {
	...
	if err != nil {
		diags = diags.Append(tfdiags.Sourceless(
			...
			fmt.Sprintf("The given host returned an API token, but Farseek failed to save it: %s.", err),
		))
	}
...
[green][bold]Success![reset] [bold]Farseek has obtained and saved an API token.[reset]

The new API token will be used for any future Farseek command that must make
...
Usage: farseek [global options] login [hostname]
...
func (c *LoginCommand) interactiveGetTokenByCode(ctx context.Context, hostname svchost.Hostname, credsCtx *loginCredentialsContext, clientConfig *disco.OAuthClient) (*oauth2.Token, tfdiags.Diagnostics) {
	...
	if c.BrowserLauncher != nil {
		...
		if err == nil {
			c.Ui.Output(fmt.Sprintf("Farseek must now open a web browser to the login page for %s.\n", hostname.ForDisplay()))
	...
	c.Ui.Output("Farseek will now wait for the host to signal that login was successful.\n")
...
func (c *LoginCommand) interactiveGetTokenByPassword(ctx context.Context, hostname svchost.Hostname, credsCtx *loginCredentialsContext, clientConfig *disco.OAuthClient) (*oauth2.Token, tfdiags.Diagnostics) {
	...
	c.Ui.Output("\n---------------------------------------------------------------------------------\n")
	c.Ui.Output("Farseek must temporarily use your password to request an API token.\nThis password will NOT be saved locally.\n")
...
func (c *LoginCommand) interactiveGetTokenByUI(ctx context.Context, hostname svchost.Hostname, credsCtx *loginCredentialsContext, service *url.URL) (svcauth.HostCredentialsToken, tfdiags.Diagnostics) {
	...
	if c.BrowserLauncher != nil {
		...
		if err == nil {
			c.Ui.Output(fmt.Sprintf("Farseek must now open a web browser to the tokens page for %s.\n", hostname.ForDisplay()))
			c.Ui.Output(fmt.Sprintf("If a browser does not open this automatically, open the following URL to proceed:\n    %s\n", tokensURL.String()))
		} else {
	...
	if credsCtx != nil {
		...
		case cliconfig.CredentialsViaHelper:
			c.Ui.Output(fmt.Sprintf("Farseek will store the token in the configured %q credentials helper\nfor use by subsequent commands.\n", credsCtx.HelperType))
		case cliconfig.CredentialsInPrimaryFile, cliconfig.CredentialsNotAvailable:
			c.Ui.Output(fmt.Sprintf("Farseek will store the token in plain text in the following file\nfor use by subsequent commands:\n    %s\n", credsCtx.LocalFilename))
		}
	}
	...
	// Update user-agent from 'go-tfe' to farseek
	cfg.Headers.Set("User-Agent", httpclient.FarseekUserAgent(version.String()))
...
func (c *LoginCommand) interactiveContextConsent(ctx context.Context, hostname svchost.Hostname, grantType disco.OAuthGrantType, credsCtx *loginCredentialsContext) (bool, tfdiags.Diagnostics) {
	...
	c.Ui.Output(fmt.Sprintf("Farseek will request an API token for %s using %s.\n", hostname.ForDisplay(), mechanism))
	...
	if credsCtx != nil {
		switch credsCtx.Location {
		case cliconfig.CredentialsViaHelper:
			c.Ui.Output(fmt.Sprintf("If login is successful, Farseek will store the token in the configured\n%q credentials helper for use by subsequent commands.\n", credsCtx.HelperType))
		case cliconfig.CredentialsInPrimaryFile, cliconfig.CredentialsNotAvailable:
			c.Ui.Output(fmt.Sprintf("If login is successful, Farseek will store the token in plain text in\nthe following file for use by subsequent commands:\n    %s\n", credsCtx.LocalFilename))
		}
	}
...
<p>The login server has returned an authentication code to Farseek.</p>
<p>Now close this page and return to the terminal where <tt>farseek login</tt>
