...
func (c *UntaintCommand) Help() string {
	helpText := `
Usage: farseek [global options] untaint [options] name

  Farseek uses the term "tainted" to describe a resource instance
  ...
  the "farseek taint" command.

  This command removes that state from a resource instance, causing
  Farseek to see it as fully-functional and not in need of
  replacement.

  This will not modify your infrastructure directly. It only avoids
  Farseek planning to replace a tainted instance in a future operation.
