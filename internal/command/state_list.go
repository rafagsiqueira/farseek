...
func (c *StateListCommand) Help() string {
	helpText := `
Usage: farseek [global options] state (list|ls) [options] [address...]

  List resources in the Farseek state.

  This command lists resource instances in the Farseek state. The address
...
Options:
  ...
  -state=statefile    Path to a Farseek state file to use to look
                      up Farseek-managed resources. By default, Farseek
...
Please ensure that your Farseek state exists and that you've
configured it properly. You can use the "-state" flag to point
Farseek at another state file.`

const errStateNotFound = `No state file was found!

State management commands require a state file. Run this command
in a directory where Farseek has been run or use the -state flag
