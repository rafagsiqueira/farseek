...
func (c *StateRmCommand) Run(args []string) int {
	...
	if isCount == 0 {
		diags = diags.Append(tfdiags.Sourceless(
			...
			"No matching objects found. To view the available instances, use \"farseek state list\". Please modify the address to reference a specific instance.",
		))
		c.showDiagnostics(diags)
		return 1
	}
...
func (c *StateRmCommand) Help() string {
	helpText := `
Usage: farseek [global options] state (remove|rm) [options] ADDRESS...

  Remove one or more items from the Farseek state, causing Farseek to
  "forget" those items without first destroying them in the remote system.

  This command removes one or more resource instances from the Farseek state
  based on the addresses given. You can view and list the available instances
  with "farseek state list".
...
Options:
  ...
  -backup=PATH            Path where Farseek should write the backup
                          state.
  ...
  -ignore-remote-version  Continue even if remote and local Farseek versions
