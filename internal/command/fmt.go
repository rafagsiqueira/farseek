...
// FmtCommand is a Command implementation that rewrites Farseek config
...
func (c *FmtCommand) fmt(paths []string, stdin io.Reader, stdout io.Writer) tfdiags.Diagnostics {
	...
	for _, path := range paths {
		...
		if info.IsDir() {
			...
			if !fmtd {
				diags = diags.Append(fmt.Errorf("Only .tf, .tfvars, and .tftest.hcl files can be processed with farseek fmt"))
				continue
			}
		}
	}
...
func (c *FmtCommand) processFile(path string, r io.Reader, w io.Writer, isStdout bool) tfdiags.Diagnostics {
	...
	log.Printf("[TRACE] farseek fmt: Formatting %s", path)
...
func (c *FmtCommand) processDir(path string, stdout io.Writer) tfdiags.Diagnostics {
	...
	log.Printf("[TRACE] farseek fmt: looking for files in %s", path)
...
func (c *FmtCommand) Help() string {
	helpText := `
Usage: farseek [global options] fmt [options] [target...]

  Rewrites all Farseek configuration files to a canonical format. All
  ...
  The content must be in the Farseek language native syntax; JSON is not
  supported.
