...
func TestApply_farseekSHA(t *testing.T) {
	// Create a temporary working directory that is empty
	td := t.TempDir()
	testCopyDir(t, testFixturePath("apply"), td)
	t.Chdir(td)
	...
	// Create .farseek_sha
	shaPath := filepath.Join(td, ".farseek_sha")
	if err := os.WriteFile(shaPath, []byte("old-sha"), 0644); err != nil {
		t.Fatal(err)
	}
	...
	farseek.Discovery = mockDiscoverer{resources: nil} // We don't care about changed resources for this test, just SHA update
	...
	// Verify .farseek_sha updated
	content, err := os.ReadFile(shaPath)
	if err != nil {
		t.Fatal(err)
	}
	if string(content) != "mock-sha" {
		t.Errorf("expected .farseek_sha to be 'mock-sha', got '%s'", string(content))
	}
}
...
func TestApply_tfWorkspaceNonDefault(t *testing.T) {
	...
	statePath := filepath.Join("farseek.tfstate.d", "test", "farseek.tfstate")
	expected := strings.TrimSpace(`
<no state>
Outputs:
...
func TestLocal_planFarseekMode_IgnoresDeletion(t *testing.T) {
	...
	config := `
	resource "test_instance" "keep_me" {
	}
	`
	...
	oldDiscovery := farseek.Discovery
	defer func() { farseek.Discovery = oldDiscovery }()
	farseek.Discovery = mockDiscovery
