	...
	for _, rawAddrFrom := range sourceAddrs {
		...
		default:
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				msgInvalidSource,
				fmt.Sprintf("Cannot move %s: Farseek doesn't know how to move this object.", rawAddrFrom),
			))
		}
...
func (c *StateMvCommand) Help() string {
	helpText := `
Usage: farseek [global options] state (move|mv) [options] SOURCE DESTINATION
 ...
 one configuration into multiple separately managed Farseek configurations.
