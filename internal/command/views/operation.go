...
import (
	...
	viewsjson "github.com/rafagsiqueira/farseek/internal/command/views/json"
...
const interrupted = `
Interrupt received.
Please wait for Farseek to exit or data loss may occur.
Gracefully shutting down...
`

const planHeaderNoOutput = `
Note: You didn't use the -out option to save this plan, so Farseek can't guarantee to take exactly these actions if you run "farseek apply" now.
`
...
Farseek has generated configuration and written it to %s. Please review the configuration and edit it as necessary before adding it to version control.
`
