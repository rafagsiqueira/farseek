...
type State struct {
	...
	// default value is blank, which is interpreted as "farseek.tfstate".
	StatePath string
	...
	case o.refreshOnlyRaw:
		...
		if !o.Refresh {
			diags = diags.Append(tfdiags.Sourceless(
				...
				"It doesn't make sense to use -refresh-only at the same time as -refresh=false, because Farseek would have nothing to do.",
			))
		}
	default:
