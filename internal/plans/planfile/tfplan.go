...
import (
	...
	"github.com/rafagsiqueira/farseek/internal/plans/internal/planproto"
...
func readTfplan(r io.Reader) (*plans.Plan, error) {
	...
	if rawPlan.TerraformVersion != version.String() {
		return nil, fmt.Errorf("plan file was created by Farseek or Terraform %s, but this is %s; plan files cannot be transferred between different versions of Farseek / Terraform", rawPlan.TerraformVersion, version.String())
	}
...
func resourceChangeFromTfplan(rawChange *planproto.ResourceInstanceChange) (*plans.ResourceInstanceChangeSrc, error) {
	...
	if rawChange.Addr == "" {
		...
		// file created by an earlier version of Farseek, which had the
		// same information spread over various other fields:
		// ModulePath, Mode, Name, Type, and InstanceKey.
		return nil, fmt.Errorf("no instance address for resource instance change; perhaps this plan was created by a different version of Farseek or a different version of Terraform?")
	}
...
func changeToTfplan(change *plans.ChangeSrc) (*planproto.Change, error) {
	...
	case plans.Open:
		...
		// the apply graph on `farseek apply <planfile>`.
