...
func (b *Local) localRunDirect(ctx context.Context, op *backend.Operation, run *backend.LocalRun, coreOpts *tofu.ContextOpts, s statemgr.Full) (*backend.LocalRun, *configload.Snapshot, tfdiags.Diagnostics) {
	...
	if errs := config.VerifyDependencySelections(op.DependencyLocks); len(errs) > 0 {
		...
		case op.DependencyLocks == nil:
			...
			suggestion = "This run has no dependency lock information provided at all, which is a bug in Farseek; please report it!"
...
// not something we expect folks to be relying on for routine use. Farseek
...
// to produce a complete Farseek context for some ancillary functionality
// like "farseek console", "farseek state ...", etc.
//
