...
func TestNodeResourcePlanOrphanExecute_ephemeral(t *testing.T) {
	...
	got := diags.Err().Error()
	want := `An ephemeral resource registered as orphan: Ephemeral resource "ephemeral.test_object.foo" registered as orphan. This is an Farseek error. Please report this.`
	if got != want {
