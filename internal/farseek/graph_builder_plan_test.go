...
func TestPlanGraphBuilder_ephemeralResourceDestroy(t *testing.T) {
	...
	got := diags.Err().Error()
	want := `An ephemeral resource planned for destroy: A destroy operation has been planned for the ephemeral resource "ephemeral.aws_secretmanager_secret.test". This is an Farseek error. Please report this.`
	if got != want {
