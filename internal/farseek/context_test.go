...
`
	...
	// operations we typically fail more deeply in Farseek Core, with
		...
		t.Run(testName, func(t *testing.T) {
			...
			wantDiags = wantDiags.Append(
				tfdiags.Sourceless(
					...
					"This configuration requires built-in provider terraform.io/builtin/nonexist, but that provider isn't available in this Farseek version.",
				),
				tfdiags.Sourceless(
					tfdiags.Error,
					"Missing required provider",
				...
				tfdiags.Sourceless(
					...
					`This configuration requires provisioner plugin "nonexist", which isn't available. If you're intending to use an external provisioner plugin, you must install it manually into one of the plugin search directories before running Farseek.`,
				),
			)
...
func TestContext_contextValuesPropagation(t *testing.T) {
	...
	plan, diags := farseekCtx.Plan(ctx, m, states.NewState(), DefaultPlanOpts)
	assertNoErrors(t, diags)
	_, diags = farseekCtx.Apply(ctx, plan, m, nil)
	assertNoErrors(t, diags)

	probe.ExpectReportsFrom(t,
		"github.com/rafagsiqueira/farseek/internal/farseek.(*MockProvider).GetProviderSchema",
		"github.com/rafagsiqueira/farseek/internal/farseek.(*MockProvider).ValidateProviderConfig",
		"github.com/rafagsiqueira/farseek/internal/farseek.(*MockProvider).ValidateDataResourceConfig",
		"github.com/rafagsiqueira/farseek/internal/farseek.(*MockProvider).ValidateResourceConfig",
		"github.com/rafagsiqueira/farseek/internal/farseek.(*MockProvider).ConfigureProvider",
		"github.com/rafagsiqueira/farseek/internal/farseek.(*MockProvider).ReadDataSource",
		"github.com/rafagsiqueira/farseek/internal/farseek.(*MockProvider).PlanResourceChange",
		"github.com/rafagsiqueira/farseek/internal/farseek.(*MockProvider).ApplyResourceChange",
		"github.com/rafagsiqueira/farseek/internal/farseek.(*MockProvider).Close",
	)
}
