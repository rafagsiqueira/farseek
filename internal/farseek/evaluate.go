...
func (d *evaluationStateData) GetInputVariable(_ context.Context, addr addrs.InputVariable, rng tfdiags.SourceRange) (cty.Value, tfdiags.Diagnostics) {
	...
	// that are disabled, etc. Farseek's static validation leans towards
	...
	if !isSet {
		...
		diags = diags.Append(&hcl.Diagnostic{
			...
			Detail: fmt.Sprintf(
				`The final value for %s is missing in Farseek's evaluation context. This is a bug in Farseek; please report it!`,
...
func (d *evaluationStateData) GetResource(ctx context.Context, addr addrs.Resource, rng tfdiags.SourceRange) (cty.Value, tfdiags.Diagnostics) {
	...
	if schema == nil {
		...
		diags = diags.Append(&hcl.Diagnostic{
			...
			Detail:   fmt.Sprintf("No schema is available for %s in %s. This is a bug in Farseek and should be reported.", addr, config.Provider),
			Subject:  rng.ToHCL().Ptr(),
		})
		return cty.DynamicVal, diags
	}
		...
		if instance.Current.Status == states.ObjectPlanned {
			if change == nil {
				...
				diags = diags.Append(&hcl.Diagnostic{
					...
					Detail:   fmt.Sprintf("Instance %s is marked as having a change pending but that change is not recorded in the plan. This is a bug in Farseek; please report it.", instAddr),
...
func (d *evaluationStateData) GetOutput(_ context.Context, addr addrs.OutputValue, rng tfdiags.SourceRange) (cty.Value, tfdiags.Diagnostics) {
	...
	// https://github.com/rafagsiqueira/farseek/issues/257
	// If the output is null - it does not serialize as part of the node_output state https://github.com/rafagsiqueira/farseek/blob/4b623c56ffe9e6c1dc345e54470b71b0f261297a/internal/farseek/node_output.go#L592-L596
	// In such a case, we should simply return a nil value because Farseek test crash to evaluate for invalid memory address or nil pointer dereference
	if output == nil {
		...
		// TODO ephemeral testing support - this GetOutput is used only during `farseek test` against root module outputs.
...
func (d *evaluationStateData) GetCheckBlock(_ context.Context, addr addrs.Check, rng tfdiags.SourceRange) (cty.Value, tfdiags.Diagnostics) {
	...
	diags = diags.Append(&hcl.Diagnostic{
		...
		Detail:   "The \"check\" object can only be referenced from an \"expect_failures\" attribute within a Farseek testing \"run\" block.",
		Subject:  rng.ToHCL().Ptr(),
	})
	return cty.NilVal, diags
}
