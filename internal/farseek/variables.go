...
// provided by the external caller into a function like farseek.Context.Plan.
//
...
// that wasn't set. Those adjustments will be handled by Farseek Core itself
// as part of performing the requested operation.
//
// A Farseek Core caller must provide an InputValue object for each of the
...
//
// Farseek Core also internally uses InputValue to represent the raw value
...
type InputValue struct {
	...
	// came from, which Farseek Core uses to tailor some of its error
...
func (v *InputValue) GoString() string {
	if (v.SourceRange != tfdiags.SourceRange{}) {
		return fmt.Sprintf("&farseek.InputValue{Value: %#v, SourceType: %#v, SourceRange: %#v}", v.Value, v.SourceType, v.SourceRange)
	} else {
		return fmt.Sprintf("&farseek.InputValue{Value: %#v, SourceType: %#v}", v.Value, v.SourceType)
	}
}
...
func (v ValueSourceType) GoString() string {
	return fmt.Sprintf("farseek.%s", v)
}
	...
	for name := range vcs {
		...
		if !isSet {
			...
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				"Unassigned variable",
				fmt.Sprintf("The input variable %q has not been assigned a value. This is a bug in Farseek; please report it in a GitHub issue.", name),
			))
			continue
		}
	}
