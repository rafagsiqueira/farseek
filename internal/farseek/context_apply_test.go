...
func TestContext2Apply_stop(t *testing.T) {
	...
	if got, want := atomic.LoadUint32(&applyStopped), uint32(1); got != want {
		...
		// to inadvertently stop Farseek before it reaches te apply phase,
...
// that might be considered user error, such as using "farseek state mv"
...
func TestContext2Apply_provisionerInterpCount(t *testing.T) {
	...
	// farseek plan -out=tfplan && farseek apply tfplan
...
// across several different Farseek subsystems. This is here because
// historically there were quirky differences in handling across different
// parts of Farseek and so here we want to assert the expected behavior and
...
func TestContext2Apply_multiVarMissingState(t *testing.T) {
	...
	// Before the relevant bug was fixed, Farseek would panic during apply.
	if _, diags := ctx.Apply(context.Background(), plan, m, nil); diags.HasErrors() {
...
func TestContext2Apply_errorDestroy(t *testing.T) {
	...
	p.PlanResourceChangeFn = func(req providers.PlanResourceChangeRequest) providers.PlanResourceChangeResponse {
		// Should actually be called for this test, because Farseek Core
...
func TestContext2Apply_errorCreateInvalidNew(t *testing.T) {
	...
	p.ApplyResourceChangeFn = func(req providers.ApplyResourceChangeRequest) providers.ApplyResourceChangeResponse {
		...
		// because we want to test that Farseek ignores the inconsistency
	...
	if got, want := len(diags), 1; got != want {
		// There should be no additional diagnostics generated by Farseek's own eval logic,
...
func TestContext2Apply_errorUpdateNullNew(t *testing.T) {
	...
	p.ApplyResourceChangeFn = func(req providers.ApplyResourceChangeRequest) providers.ApplyResourceChangeResponse {
		...
		// test that Farseek retains the prior state, rather than treating
...
func TestContext2Apply_targetedDestroy(t *testing.T) {
	...
	// test did not match actual farseek behavior: the output remains in
	// state.
	//
...
func TestContext2Apply_vars(t *testing.T) {
	...
	variables := InputValues{
		...
		"bar": &InputValue{
			...
			// has a declared default, which Farseek Core will use instead.
			Value:      cty.NilVal,
			SourceType: ValueFromCaller,
		},
