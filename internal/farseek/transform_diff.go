	...
	for _, rc := range changes.Resources {
		...
		if dk != states.NotDeposed && rc.Action != plans.Delete && rc.Action != plans.NoOp && rc.Action != plans.Forget {
			diags = diags.Append(tfdiags.Sourceless(
				...
				fmt.Sprintf("The plan contains a non-removal change for %s deposed object %s. The only valid actions for a deposed object is to destroy it or forget it, so this is a bug in Farseek.", addr, dk),
			))
			continue
		}
			...
			if state != nil {
				...
				if ris == nil {
					...
					diags = diags.Append(tfdiags.Sourceless(
						...
						fmt.Sprintf("The plan contains a replace change for %s, which doesn't exist yet. This is a bug in Farseek.", addr),
					))
					continue
				}
		...
		if update {
			...
			if createBeforeDestroy {
				...
				// For more details, see the issue https://github.com/rafagsiqueira/farseek/issues/2398
