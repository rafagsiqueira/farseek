...
type NodePlannableResourceInstance struct {
	*NodeAbstractResourceInstance
	...
	// FarseekMode indicates that we should force refreshes for targeted resources
	FarseekMode bool
}
...
func (n *NodePlannableResourceInstance) managedResourceExecute(ctx context.Context, evalCtx EvalContext) (diags tfdiags.Diagnostics) {
	...
	if importing && n.Config == nil && len(n.generateConfigPath) == 0 && !n.FarseekMode {
		...
		if n.Addr.Module.IsRoot() {
			diags = diags.Append(&hcl.Diagnostic{
				...
				Detail:   "The target for the given import block does not exist. If you wish to automatically generate config for this resource, use the -generate-config-out option within farseek plan. Otherwise, make sure the target resource exists within your configuration. For example:\n\n  farseek plan -generate-config-out=generated.tf",
	...
	if importing {
		...
		// FARSEEK: Check First
		// In stateless mode, if the resource is missing from state, we try to see if it
		// already exists in the cloud by speculatively importing it using the 'name'
		// attribute from its configuration.
		if instanceRefreshState == nil && n.FarseekMode && n.Config != nil && n.Config.Managed != nil {
			schema, _ := providerSchema.SchemaForResourceAddr(addr.Resource.Resource)
			if schema != nil {
				// Evaluate the block to get the 'name' attribute
				expander := evalCtx.InstanceExpander()
				repData := expander.GetResourceInstanceRepetitionData(n.Addr)
				val, _, evalDiags := evalCtx.EvaluateBlock(ctx, n.Config.Config, schema, nil, repData)
				if !evalDiags.HasErrors() && !val.IsNull() {
					attrs := val.AsValueMap()
					if nameVal, ok := attrs["name"]; ok && !nameVal.IsNull() && nameVal.Type() == cty.String && nameVal.IsKnown() {
						id := nameVal.AsString()
						log.Printf("[INFO] Farseek: Speculatively importing %s with ID %q (Check First)", addr, id)
						s, impDiags := n.importState(ctx, evalCtx, addr, id, provider, providerSchema)
						if !impDiags.HasErrors() && s != nil {
							instanceRefreshState = s
							log.Printf("[INFO] Farseek: Successfully speculatively imported %s", addr)
						}
					}
				}
			}
	...
	if !n.skipRefresh && !importing {
		if instanceRefreshState == nil && n.FarseekMode {
			// In Farseek mode (stateless), we force a refresh for targeted resources
			// even if there's no state. This allows selective cloud polling.
			isTargeted := false
			addr := n.ResourceInstanceAddr()
			for _, t := range n.Targets {
				if t.TargetContains(addr) {
					isTargeted = true
					break
				}
			}
			...
			if isTargeted {
				// In Farseek mode (stateless), we force a refresh for targeted resources
				// even if there's no state. This allows selective cloud polling.
				log.Printf("[DEBUG] Farseek: forcing refresh for targeted resource %s", addr)
				schema, _ := providerSchema.SchemaForResourceAddr(addr.Resource.Resource)
				if schema != nil {
					...
					log.Printf("[ERROR] Farseek: Schema not found for %s during forced refresh", addr)
				}
			}
		}
	...
	if !n.skipPlanChanges {
		...
		// reachable via the farseek.EvalContext API.
		diags = diags.Append(n.writeChange(ctx, evalCtx, change, ""))
		if diags.HasErrors() {
			return diags
		}
		...
		diags = diags.Append(tfdiags.Sourceless(
			...
			fmt.Sprintf(
				"While attempting to import an existing object to %q, "+
					...
					"configured region or endpoint, or use \"farseek apply\" to "+
