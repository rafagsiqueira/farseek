...
import (
	...
	"github.com/rafagsiqueira/farseek/internal/lang/globalref"
	"github.com/rafagsiqueira/farseek/internal/plans"
	"github.com/rafagsiqueira/farseek/internal/refactoring"
...
// PlanOpts are the various options that affect the details of how Farseek
// will build a plan.
type PlanOpts struct {
	...
	// mode, where Farseek will take actions only for resource instances
	...
	// and so populating this field will cause Farseek to generate extra
	...
	//
	// A typical use of this argument is to ask Farseek to replace an object
	// which the user has determined is somehow degraded (via information from
	// outside of Farseek), thereby hopefully replacing it with a
	...
	// GenerateConfig tells Farseek where to write any generated configuration
	// for any ImportTargets that do not have configuration already.
	//
	// If empty, then no config will be generated.
	GenerateConfigPath string

	// FarseekMode indicates that we are running in Farseek mode (stateless).
	FarseekMode bool
}
...
// You can use farseek.DefaultPlanOpts to generate a normal plan with no
// special options.
//
// If the returned diagnostics contains no errors then the returned plan is
// applyable, although Farseek cannot guarantee that applying it will fully
...
func (c *Context) Plan(ctx context.Context, config *configs.Config, prevRunState *states.State, opts *PlanOpts) (*plans.Plan, tfdiags.Diagnostics) {
	...
	case plans.RefreshOnlyMode:
		if opts.SkipRefresh {
			...
			diags = diags.Append(tfdiags.Sourceless(
				...
				"Cannot skip refreshing in refresh-only mode. This is a bug in Farseek.",
			))
			return nil, diags
		}
	default:
		// The CLI layer (and other similar callers) should not try to
		// create a context for a mode that Farseek Core doesn't support.
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			"Unsupported plan mode",
			fmt.Sprintf("Farseek Core doesn't know how to handle plan mode %s. This is a bug in Farseek.", opts.Mode),
		))
		return nil, diags
	}
	...
	if (len(opts.Targets) > 0 || len(opts.Excludes) > 0) && !opts.FarseekMode {
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Warning,
...
The -target and -exclude options are not for routine use, and are provided only for exceptional situations such as recovering from errors or mistakes, or when Farseek specifically suggests to use it as part of an error message.`,
		))
	}
...
// the graph. This allows farseek to report errors (mostly cycles) during
...
// handling real user requests to run Farseek, it'd probably be better
...
func (c *Context) refreshOnlyPlan(ctx context.Context, config *configs.Config, prevRunState *states.State, opts *PlanOpts) (*plans.Plan, tfdiags.Diagnostics) {
	...
	if len(plan.Changes.Resources) != 0 {
		...
		diags = diags.Append(tfdiags.Sourceless(
			...
			"Farseek generated planned resource changes in a refresh-only plan. This is a bug in Farseek.",
		))
	}
...
func (c *Context) destroyPlan(ctx context.Context, config *configs.Config, prevRunState *states.State, opts *PlanOpts) (*plans.Plan, tfdiags.Diagnostics) {
	...
	if !opts.SkipRefresh && !prevRunState.Empty() {
		...
		// detected during refreshing that anything was already deleted outside Farseek.
...
func (c *Context) prePlanVerifyMovesWithTargetFlag(moveResults refactoring.MoveResults, targets []addrs.Targetable) tfdiags.Diagnostics {
	...
	if len(excluded) > 0 {
		...
		diags = diags.Append(tfdiags.Sourceless(
			...
			fmt.Sprintf(
				"Resource instances in your current state have moved to new addresses in the latest configuration. Farseek must include those resource instances while planning in order to ensure a correct result, but your -target=... options do not fully cover all of those resource instances.\n\nTo create a valid plan, either remove your -target=... options altogether or add the following additional target options:%s\n\nNote that adding these options may include further additional resource instances in your plan, in order to respect object dependencies.",
				listBuf.String(),
			),
		))
	}
...
func (c *Context) prePlanVerifyMovesWithExcludeFlag(moveResults refactoring.MoveResults, excludes []addrs.Targetable) tfdiags.Diagnostics {
	...
	if len(excluded) > 0 {
		...
		diags = diags.Append(tfdiags.Sourceless(
			...
			"Moved resource instances excluded by targeting",
			fmt.Sprintf(
				"Resource instances in your current state have moved to new addresses in the latest configuration. Farseek must include those resource instances while planning in order to ensure a correct result, but your -exclude=... options exclude some of those resource instances.\n\nTo create a valid plan, either remove your -exclude=... options altogether or just specifically remove the following options:%s\n\nNote that removing these options may include further additional resource instances in your plan, in order to respect object dependencies.",
				listBuf.String(),
			),
		))
	}
	...
	for _, ms := range oldState.Modules {
		for _, rs := range ms.Resources {
			...
			for key, oldIS := range rs.Instances {
				...
				//
				// - Resources which were deleted outside Farseek;
				// - Resources where the object value has changed outside Farseek;
...
// by the "farseek graph" command when asked to render a plan-time graph.
//
...
// in that context, even if drifts away from the physical graph that Farseek
...
func blockedMovesWarningDiag(results refactoring.MoveResults) tfdiags.Diagnostic {
	...
	return tfdiags.Sourceless(
		...
		fmt.Sprintf(
			"Farseek tried to adjust resource instance addresses in the prior state based on change information recorded in the configuration, but some adjustments did not succeed due to existing objects already at the intended addresses:%s\n\nFarseek has planned to destroy these objects. If Farseek's proposed changes aren't appropriate, you must first resolve the conflicts using the \"farseek state\" subcommands and then create a new plan.",
			itemsBuf.String(),
		),
	)
}
