...
func TestEvaluatorGetTerraformAttr(t *testing.T) {
	...
	t.Run("farseek.workspace", func(t *testing.T) {
		want := cty.StringVal("foo")
		got, diags := scope.Data.GetTerraformAttr(t.Context(), addrs.NewTerraformAttr("farseek", "workspace"), tfdiags.SourceRange{})
		if len(diags) != 0 {
			t.Errorf("unexpected diagnostics %s", spew.Sdump(diags))
		}
		if !got.RawEquals(want) {
	...
	t.Run("farseek.applying", func(t *testing.T) {
		want := cty.False.Mark(marks.Ephemeral)
		got, diags := scope.Data.GetTerraformAttr(t.Context(), addrs.NewTerraformAttr("farseek", "applying"), tfdiags.SourceRange{})
		if len(diags) != 0 {
			t.Errorf("unexpected diagnostics %s", spew.Sdump(diags))
		}
		if !got.RawEquals(want) {
