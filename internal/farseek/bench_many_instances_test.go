...
//
//	go test ./internal/farseek -bench='^BenchmarkManyResourceInstances$' -benchtime=1x
func BenchmarkManyResourceInstances(b *testing.B) {
	...
	farseekCtx := testContext2(b, &ContextOpts{
		...
		// Farseek's direct overheads and not by the artificial concurrency
		// limit.
	...
	for range b.N {
		...
		plan, planDiags := farseekCtx.Plan(ctx, m, priorState, planOpts)
		assertNoDiagnostics(b, planDiags)

		_, applyDiags := farseekCtx.Apply(ctx, plan, m, nil)
...
//
//	go test ./internal/farseek -bench='^BenchmarkManyModuleInstances$' -benchtime=1m
func BenchmarkManyModuleInstances(b *testing.B) {
	...
	for range b.N {
		plan, planDiags := farseekCtx.Plan(ctx, m, states.NewState(), planOpts)
		assertNoDiagnostics(b, planDiags)
