...
// ApplyOpts are the various options that affect the details of how Farseek
// will build a plan.
//
		...
		if forgetCount > 0 {
			...
			if !suppressError {
				diags = diags.Append(tfdiags.Sourceless(
					...
					`As requested, Farseek has not deleted some remote objects that are no longer managed by this configuration. Those objects continue to exist in their remote system and so may continue to incur charges. Refer to the original plan for more information.
To suppress this error for the future 'destroy' runs, you can add the CLI flag "-suppress-forget-errors".`,
	...
	if (len(plan.TargetAddrs) > 0 || len(plan.ExcludeAddrs) > 0) && !plan.FarseekMode {
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Warning,
...
Note that the -target and -exclude options are not suitable for routine use, and are provided only for exceptional situations such as recovering from errors or mistakes, or when Farseek specifically suggests to use it as part of an error message.`,
		))
	}
...
// use by the "farseek graph" command when asked to render an apply-time
// graph.
//
...
// Therefore, when a plan is created via `farseek plan -out <planfile>` and then applied with `farseek apply <planfile>`
...
func (c *Context) mergePlanAndApplyVariables(config *configs.Config, plan *plans.Plan, opts *ApplyOpts) (InputValues, tfdiags.Diagnostics) {
	...
	for name, cfg := range config.Module.Variables {
		...
		// which case Farseek will once again use the default value from the
		...
		// * Will be in the plan when `farseek apply` will be executed without a plan file
		// * Will be in the applyOpts when `farseek apply` will be executed with a plan file
		if planOk && inputOk {
		...
		if plan.EphemeralVariables[name] && cfg.Required() && !inputOk && !planOk {
			...
			diags = diags.Append(&hcl.Diagnostic{
				...
				Detail:   fmt.Sprintf("Variable %q is configured as ephemeral. This type of variables need to be given a value during `farseek plan` and also during `farseek apply`.", name),
