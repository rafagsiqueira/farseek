...
// callback. This allows you to modify the behavior of Farseek at runtime.
...
const (
	...
	// and the action that Farseek was about to take is cancelled.
	HookActionHalt
)

// Hook is the interface that must be implemented to hook into various
// parts of Farseek, allowing you to inspect or change behavior at runtime.
//
// There are MANY hook points into Farseek. If you only want to implement
...
type Hook interface {
	...
	// Stopping is called if an external signal requests that Farseek
	// gracefully abort an operation in progress.
	//
	// This notification might suggest that the user wants Farseek to exit
	// ASAP and in that case it's possible that if Farseek runs for too much
	...
	//
	// This hook cannot control whether Farseek continues, because the
