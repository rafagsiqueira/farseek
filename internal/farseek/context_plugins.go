...
// farseek.Context, and thus it'll be safe to cache certain information
...
// parts of Farseek all need the same schema information.
func (cp *contextPlugins) ProviderSchema(ctx context.Context, addr addrs.Provider) (providers.ProviderSchema, error) {
	...
	if !ok {
		entry = sync.OnceValues(func() (providers.ProviderSchema, error) {
			log.Printf("[TRACE] farseek.contextPlugins: Initializing provider %q to read its schema", addr)
...
func (cp *contextPlugins) ProvisionerSchema(addr string) (*configschema.Block, error) {
	...
	if !ok {
		entry = sync.OnceValues(func() (*configschema.Block, error) {
			log.Printf("[TRACE] farseek.contextPlugins: Initializing provisioner %q to read its schema", addr)
