	...
	t.Run("unused object attribute", func(t *testing.T) {
		n := &nodeModuleVariable{
			...
			Expr: &hclsyntax.ObjectConsExpr{
				SrcRange: hcl.Range{Filename: "context1.farseek"},
				Items: []hclsyntax.ObjectConsItem{
					{
						KeyExpr: &hclsyntax.LiteralValueExpr{
							Val: cty.StringVal("baz"),
							SrcRange: hcl.Range{
								Filename: "test1.farseek",
							},
						},
					...
					},
					{
						KeyExpr: &hclsyntax.LiteralValueExpr{
							...
							SrcRange: hcl.Range{
								Filename: "test.farseek",
							},
						},
						ValueExpr: &hclsyntax.ObjectConsExpr{
							SrcRange: hcl.Range{Filename: "context2.farseek"},
							Items: []hclsyntax.ObjectConsItem{
								{
									KeyExpr: &hclsyntax.LiteralValueExpr{
										Val: cty.StringVal("beep"),
										SrcRange: hcl.Range{
											Filename: "test2.farseek",
										},
									},
									ValueExpr: &hclsyntax.LiteralValueExpr{
										Val: cty.StringVal("..."),
		...
		wantDiags = wantDiags.Append(&hcl.Diagnostic{
			...
			Subject: &hcl.Range{
				Filename: "test1.farseek", // from synthetic source range in constructed expression above
			},
			Context: &hcl.Range{
				Filename: "context1.farseek", // from synthetic source range in constructed expression above
			},
		})
		wantDiags = wantDiags.Append(&hcl.Diagnostic{
			Severity: hcl.DiagWarning,
			...
			Subject: &hcl.Range{
				Filename: "test2.farseek", // from synthetic source range in constructed expression above
			},
			Context: &hcl.Range{
				Filename: "context2.farseek", // from synthetic source range in constructed expression above
			},
		})
