...
func TestEvaluateImportIdExpression_SensitiveValue(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			wantErr: "Invalid import id argument: The import block \"id\" argument depends on resource attributes that cannot be determined until apply, so Farseek cannot plan to import this resource.", // Adapted the message from your original code
		},
		{
			name:    "valid_value",
