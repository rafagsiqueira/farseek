...
// into Farseek in various ways. This must be provided by the Context
// initializer.
type ContextMeta struct {
	Env string // Env is the state environment

	// OriginalWorkingDir is the working directory where the Farseek CLI
...
// Context represents all the context that Farseek needs in order to
...
func NewContext(opts *ContextOpts) (*Context, tfdiags.Diagnostics) {
	...
	log.Printf("[TRACE] farseek.NewContext: starting")
	...
	log.Printf("[TRACE] farseek.NewContext: complete")
...
func (c *Context) Stop() {
	log.Printf("[WARN] farseek: Stop called, initiating interrupt sequence")
	...
	if c.runContextCancel != nil {
		log.Printf("[WARN] farseek: run context exists, stopping")
	...
	if cond := c.runCond; cond != nil {
		log.Printf("[INFO] farseek: waiting for graceful stop to complete")
		cond.Wait()
	}

	log.Printf("[WARN] farseek: stop complete")
}
	...
	go func() {
...

		{
			...
			for _, p := range toStop {
				...
				// advisory: Farseek will exit once the graph node completes.
...
//
// Currently this function checks whether the current Farseek CLI version
...
func (c *Context) checkConfigDependencies(config *configs.Config) tfdiags.Diagnostics {
	...
	// This checks the Farseek CLI version constraints specified in all of
	// the modules.
	...
	for providerAddr := range providerReqs {
		if !c.plugins.HasProvider(providerAddr) {
			if !providerAddr.IsBuiltIn() {
				...
				// Built-in providers can never be installed by "farseek init",
				// so no point in confusing the user by suggesting that.
				diags = diags.Append(tfdiags.Sourceless(
					tfdiags.Error,
					"Missing required provider",
					fmt.Sprintf(
						"This configuration requires built-in provider %s, but that provider isn't available in this Farseek version.",
						providerAddr,
					),
				))
			}
		...
		for _, rc := range modCfg.Module.ManagedResources {
			...
			for _, pc := range rc.Managed.Provisioners {
				if !c.plugins.HasProvisioner(pc.Type) {
					...
					// the caller of farseek.NewContext should've already
					...
					diags = diags.Append(tfdiags.Sourceless(
						...
						fmt.Sprintf(
							"This configuration requires provisioner plugin %q, which isn't available. If you're intending to use an external provisioner plugin, you must install it manually into one of the plugin search directories before running Farseek.",
							pc.Type,
						),
					))
				}
