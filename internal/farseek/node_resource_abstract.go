...
type NodeAbstractResource struct {
	...
	// FarseekMode indicates that we should allow refreshing/planning
	// even if the resource is missing from the configuration.
	FarseekMode bool
}
...
func (n *NodeAbstractResourceInstance) readResourceInstanceState(ctx context.Context, evalCtx EvalContext, addr addrs.AbsResourceInstance) (*states.ResourceInstanceObject, tfdiags.Diagnostics) {
	...
	if schema == nil {
		...
		return nil, diags.Append(fmt.Errorf("no schema available for %s while reading state; this is a bug in Farseek and should be reported", addr))
	}
...
func (n *NodeAbstractResourceInstance) readResourceInstanceStateDeposed(ctx context.Context, evalCtx EvalContext, addr addrs.AbsResourceInstance, key states.DeposedKey) (*states.ResourceInstanceObject, tfdiags.Diagnostics) {
	...
	if key == states.NotDeposed {
		return nil, diags.Append(fmt.Errorf("readResourceInstanceStateDeposed used with no instance key; this is a bug in Farseek and should be reported"))
	}
