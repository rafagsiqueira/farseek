	...
	if path.IsRoot() && c.Module.ProviderRequirements != nil {
		for name, p := range c.Module.ProviderRequirements.RequiredProviders {
			...
			if t.Operation == walkValidate {
				// Add a workaround for validating modules by running them as a root module in `farseek validate`
				// See the discussion in https://github.com/rafagsiqueira/farseek/issues/2862 for more details
