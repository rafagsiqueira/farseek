...
func TestContext2Validate_providerAliasesInRoot(t *testing.T) {
	// This tests the scenario where a user is running farseek validate in a module, instead of the root
...
func TestContext2Validate_importWithForEachOnUnknown(t *testing.T) {
	...
	// Related to https://github.com/rafagsiqueira/farseek/issues/3563
	m := testModuleInline(t, map[string]string{
		"main.tf": `
			variable "server_ids" {
			  type    = list(string)
...
func TestContext2Validate_importIntoModuleResource(t *testing.T) {
	...
	// Related to: https://github.com/rafagsiqueira/farseek/issues/3562
...
func TestContext2Validate_importIntoUnexistingResourceBlock(t *testing.T) {
	...
	// Related to: https://github.com/rafagsiqueira/farseek/issues/3615
	p := simpleMockProvider()
	hook := new(MockHook)
	ctx := testContext2(t, &ContextOpts{
		Hooks: []Hook{hook},
