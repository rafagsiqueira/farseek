...
func TestDiffTransformerEphemeralChanges(t *testing.T) {
	...
	got := diags.Err().Error()
	want := "Destroy invoked for an ephemeral resource: A destroy operation has been invoked for the ephemeral resource \"ephemeral.aws_secretmanager_secret.foo\". This is an Farseek error. Please report this."
	if got != want {
