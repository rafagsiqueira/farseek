...
type ConfigTransformer struct {
	...
	// FarseekMode indicates that we are in stateless mode and should
	// allow importing resources even if they are missing from the configuration.
	FarseekMode bool
}
...
func (t *ConfigTransformer) transformSingle(g *Graph, config *configs.Config, generateConfigPath string) error {
	...
	for _, i := range importTargets {
		...
		case t.FarseekMode:
			// Create a node with the resource and import target.
			abstract := &NodeAbstractResource{
				Addr:          i.StaticAddr(),
				importTargets: []*ImportTarget{i},
				FarseekMode:   true,
