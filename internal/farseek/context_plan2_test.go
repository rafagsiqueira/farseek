...
func TestContext2Plan_removedDuringRefresh(t *testing.T) {
	...
	// state has been deleted outside Farseek, which we should detect
...
`})
	...
	// Farseek that made it invalid, although technically in that scenario
...
func TestContext2Plan_managedResourceChecksOtherManagedResourceChange(t *testing.T) {
	...
	// This doesn't work because Farseek can't tell that the data resource
...
locals {
	...
	# cause Farseek to defer the data resource to the apply phase due to
	...
	# so Farseek can't "see" that the data resource result might be affected
...
func TestContext2Plan_movedResourceCollision(t *testing.T) {
	...
	wantDiags := tfdiags.Diagnostics{
		tfdiags.Sourceless(
			...
			`Farseek tried to adjust resource instance addresses in the prior state based on change information recorded in the configuration, but some adjustments did not succeed due to existing objects already at the intended addresses:
...
func TestContext2Plan_movedResourceCollisionDestroy(t *testing.T) {
	...
	wantDiags := tfdiags.Diagnostics{
		tfdiags.Sourceless(
			...
			// because it says "Farseek has planned to destroy these objects"
			...
			// a "farseek destroy", so we'll just live with that for now
...
`,
	...
	for _, diag := range diags {
		...
		case "Error message refers to sensitive values":
			if got, want := desc.Detail, "The error expression used to explain this condition refers to sensitive values, so Farseek will not display the resulting message."; !strings.Contains(got, want) {
				t.Errorf("unexpected detail\ngot: %s\nwant to contain %q", got, want)
			}
		default:
			t.Errorf("unexpected summary\ngot: %s", desc.Summary)
...
`,
		...
		},
		{
			...
			expectedError: `The "for_each" value includes keys or set values from resource attributes that cannot be determined until apply, and so Farseek cannot determine what will identify the instances of this resource.`,
			inlineConfiguration: map[string]string{
				"main.tf": `
resource "test_object" "reference" {
}
...
`,
		...
		},
		{
			...
			expectedError: `Invalid import id argument: The import block "id" argument depends on resource attributes that cannot be determined until apply, so Farseek cannot plan to import this resource.`,
			inlineConfiguration: map[string]string{
				"main.tf": `
resource "test_object" "reference" {
}
...
`,
		...
		},
		{
			...
			expectedError: `Invalid for_each argument: The "for_each" set includes values derived from resource attributes that cannot be determined until apply, and so Farseek cannot determine the full set of keys that will identify the instances of this resource.`,
			inlineConfiguration: map[string]string{
				"main.tf": `
resource "test_object" "reference" {
}
...
func TestContext2Plan_providerForEachWithOrphanResourceInstanceNotUsingForEach(t *testing.T) {
	...
	//    https://github.com/rafagsiqueira/farseek/issues/2334
	//
	// The bug there was that Farseek was failing to evaluate the provider
	...
	farseekCtx := testContext2(t, &ContextOpts{
		Providers: map[addrs.Provider]providers.Factory{
			providerConfigAddr.Provider: testProviderFuncFixed(p),
		},
	})
	_, diags := farseekCtx.Plan(context.Background(), m, s, DefaultPlanOpts)
	err := diags.Err()
	if err == nil {
