...
func (n *NodePlanDeposedResourceInstanceObject) Execute(ctx context.Context, evalCtx EvalContext, op walkOperation) (diags tfdiags.Diagnostics) {
	...
	if !n.skipRefresh && op != walkPlanDestroy {
		...
		// case it's already been deleted outside Farseek. If this is a
		...
		// Farseek is expected to remove the missing resource from the state
		// entirely
	...
	if !n.skipPlanChanges {
		...
		if shouldDestroy {
			...
			diags = diags.Append(&hcl.Diagnostic{
				...
				Detail:   fmt.Sprintf("After this plan is applied, the resource %s will not be managed anymore by Farseek.\n\nIn case you want to manage the resource again, you will have to import it.", n.Addr),
			})
...
func (n *NodeDestroyDeposedResourceInstanceObject) writeResourceInstanceState(ctx context.Context, evalCtx EvalContext, obj *states.ResourceInstanceObject) error {
	...
	if key == states.NotDeposed {
		// should never happen
		return fmt.Errorf("can't save deposed object for %s without a deposed key; this is a bug in Farseek that should be reported", absAddr)
	}
