...
// TestContext2Apply_sensitiveInsideUnknown verifies that Farseek uses
...
//
// For additional context, refer to https://github.com/rafagsiqueira/farseek/issues/3367 .
func TestContext2Apply_sensitiveInsideUnknown(t *testing.T) {
	...
	plan, diags := farseekCtx.Plan(t.Context(), m, states.NewState(), SimplePlanOpts(plans.NormalMode, nil))
	assertNoErrors(t, diags)
	state, diags := farseekCtx.Apply(context.Background(), plan, m, &ApplyOpts{})
	assertNoErrors(t, diags)
...
func TestContext2Apply_resourceConditionApplyTimeFail(t *testing.T) {
	...
	//
	// This edge case is a tricky one because it relies on Farseek still
...
func TestContext2Apply_providerExpandWithTargetOrExclude(t *testing.T) {
	...
	// cause Farseek to skip visiting certain objects.
	//
	...
	// each test.
	// For correct operation the plan options must cause Farseek to
	...
	runTest := func(t *testing.T, makeStep2PlanOpts func(plans.Mode) *PlanOpts) {
		...
		// to how Farseek CLI would do it, in case its normalization rules and
...
`
	complete := testModuleInline(t, map[string]string{
		"locals.farseek":    localComplete,
		"providers.farseek": providerConfig,
		"resources.farseek": resourceConfig,
	})
	partial := testModuleInline(t, map[string]string{
		"locals.farseek":    localPartial,
		"providers.farseek": providerConfig,
		"resources.farseek": resourceConfig,
	})
...
`
	complete := testModuleInline(t, map[string]string{
		"locals.farseek":        localComplete,
		"providers.farseek":     providerConfig,
		"modules.farseek":       moduleCall,
		"mod/resources.farseek": resourceConfig,
	})
	partial := testModuleInline(t, map[string]string{
		"locals.farseek":        localPartial,
		"providers.farseek":     providerConfig,
...
check "http_check" {
  ...
  assert {
    condition     = data.http.farseek.status_code == 200
    error_message = "${data.http.farseek.url} returned an unhealthy status code"
  }
}
`,
	})
...
`,
	...
	cases := map[string]struct {
		...
		applyOpts        *ApplyOpts
		// Set this to "true" to test a flow similar with the one ran when running `farseek plan -out <planfile>` followed by `farseek apply <planfile>`.
		// "False" it means that it will run the test like running directly `farseek apply -auto-approve`
		...
		// then read from the file during `farseek apply <planfile>`. This is the only way for the
		...
		"mutate plan to be similar with the one loaded from file and apply without any opts": {
			...
			applyOpts:             nil,
			expectedApplyErrors:   []string{"No value for required variable - Variable \"ephemeral_required\" is configured as ephemeral. This type of variables need to be given a value during `farseek plan` and also during `farseek apply`."},
		},
		...
		// These are not reproducible IRL because in a direct `farseek apply` command, the ApplyOpts and PlanOpts will not exist together.
			...
			[]tfdiags.Description{
				{
					...
					Detail:  "Variable \"var1\" is configured as ephemeral. This type of variables need to be given a value during `farseek plan` and also during `farseek apply`.",
				},
			},
		},
...
// be executed.
// This test has been added when a fix for https://github.com/rafagsiqueira/farseek/issues/3489
// was provided.
