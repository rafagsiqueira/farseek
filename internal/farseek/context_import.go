...
// IsFromImportCommandLine checks whether the import target originates from a `farseek import` command
...
// under Farseek management. Import requires the exact type, name, and ID
...
// ImportStateless is like Import but it doesn't validate that the imported
// resources have configuration. This is used in stateless modes where we
// want to import resources even if they are missing from the current configuration
// (e.g. to plan their destruction).
func (c *Context) ImportStateless(ctx context.Context, config *configs.Config, prevRunState *states.State, opts *ImportOpts) (*states.State, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
	...
	// SKIP postExpansionImportValidation for stateless mode
