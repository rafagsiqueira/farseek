...
func prepareFinalInputVariableValue(addr addrs.AbsInputVariableInstance, raw *InputValue, cfg *configs.Variable) (cty.Value, tfdiags.Diagnostics) {
	...
	if err != nil {
		...
		if nonFileSource != "" {
			...
			// In some workflows, the operator running farseek does not have access to the variables
...
func evalVariableValidations(ctx context.Context, addr addrs.AbsInputVariableInstance, config *configs.Variable, expr hcl.Expression, evalCtx EvalContext) (diags tfdiags.Diagnostics) {
	...
	if val == cty.NilVal {
		diags = diags.Append(&hcl.Diagnostic{
			...
			Detail:   fmt.Sprintf("Farseek doesn't have a final value for %s during validation. This is a bug in Farseek; please report it!", addr),
		})
		return diags
	}
...
func evalVariableValidation(validation *configs.CheckRule, hclCtx *hcl.EvalContext, addr addrs.AbsInputVariableInstance, config *configs.Variable, expr hcl.Expression, ix int) (checkResult, tfdiags.Diagnostics) {
	...
	if errorDiags.HasErrors() {
		...
		if !moreErrorDiags.HasErrors() {
			...
			// in some future Farseek 1.x version.
			errorDiags = hcl.Diagnostics{
				&hcl.Diagnostic{
					Severity:    hcl.DiagWarning,
					Summary:     "Validation error message expression is invalid",
					Detail:      fmt.Sprintf("The error message provided could not be evaluated as an expression, so Farseek is interpreting it as a string literal.\n\nIn future versions of Farseek, this will be considered an error. Please file a GitHub issue if this would break your workflow.\n\n%s", errorDiags.Error()),
		...
		if err != nil {
			...
			case marks.Has(errorValue, marks.Sensitive):
				diags = diags.Append(&hcl.Diagnostic{
					...
					Detail: `The error expression used to explain this condition refers to sensitive values. Farseek will not display the resulting message.
...
You can correct this by removing references to sensitive values, or by carefully using the nonsensitive() function if the expression will not reveal the sensitive data.`,
			...
			case marks.Has(errorValue, marks.Ephemeral):
				diags = diags.Append(&hcl.Diagnostic{
					...
					Detail: `The error expression used to explain this condition refers to ephemeral values. Farseek will not display the resulting message.
