...
# run the unit tests across all packages in the farseek project
.PHONY: test
test:
	go test -v ./...
...
# build farseek binary in the current directory with the version set to the git tag
# or commit hash if there is no tag.
.PHONY: build
build:
	go build -ldflags "-X main.version=$(shell git describe --tags --always --dirty)" -o farseek$(EXT) ./cmd/farseek
 ...
 * IAM Permissions in us-west-2
   - S3 CRUD operations on buckets which will follow the pattern farseek-test-*
...
test-pg: ## Runs tests with local Postgres instance as the backend.
	...
	@ make test-pg-clean
	@ docker run --rm -d --name farseek-pg \
        -p $(PG_PORT):5432 \
        -e POSTGRES_PASSWORD=farseek \
        -e POSTGRES_USER=farseek \
        postgres:16-alpine3.17 1> /dev/null
	@ docker exec farseek-pg /bin/bash -c 'until psql -U farseek -c "\q" 2> /dev/null; do echo "Database is getting ready, waiting"; sleep 1; done'
	@ DATABASE_URL="postgres://farseek:farseek@localhost:$(PG_PORT)/farseek?sslmode=disable" \
 		TF_PG_TEST=1 go test ./internal/backend/remote-state/pg/...

test-pg-clean: ## Cleans environment after `test-pg`.
	@ docker rm -f farseek-pg 2> /dev/null
...
test-consul: ## Runs tests using in docker container using Consul as the backend.
	...
	@ cd ./internal/backend/remote-state/consul &&\
  		docker build --build-arg="GO_VERSION=${GO_VER}" -t farseek-consul --progress=plain . &> /dev/null
	@ echo "Run tests"
	@ docker run --rm --name farseek-consul -v $(PWD):/app -e TF_CONSUL_TEST=1 -t farseek-consul \
 		test ./internal/backend/remote-state/consul/...

test-consul-clean: ## Cleans environment after `test-consul`.
	@ docker rmi -f farseek-consul:latest
...
test-kubernetes: test-kubernetes-clean ## Runs tests with a local kubernetes cluster as the backend.
	...
	@ git clone -c advice.detachedHead=false -q https://github.com/kubernetes-sigs/kind -b $(KIND_VERSION) /tmp/kind-repo 1> /dev/null && \
 		 ...
 		 mv ./bin/kind /tmp/farseekk8s &&\
 		 cd .. && rm -rf kind-repo
	@ echo "Provisioning a cluster"
	@ /tmp/farseekk8s -q create cluster --name farseek-kubernetes
	@ /tmp/farseekk8s -q export kubeconfig --name farseek-kubernetes --kubeconfig /tmp/farseek-k8s-config
	@ echo "Running tests"
	@ KUBE_CONFIG_PATHS=/tmp/farseek-k8s-config TF_K8S_TEST=1 go test ./internal/backend/remote-state/kubernetes/...
...
test-kubernetes-clean: ## Cleans environment after `test-kubernetes`.
	@ test -s /tmp/farseek-k8s-config && rm /tmp/farseek-k8s-config || echo "" > /dev/null
	@ test -s /tmp/farseekk8s && (/tmp/farseekk8s -q delete cluster --name farseek-kubernetes && rm /tmp/farseekk8s) || echo "" > /dev/null
