...
const (
	...
	EnvCLI = "FARSEEK_CLI_ARGS"

	// The parent process will create a file to collect crash logs
	envTmpLogPath = "FARSEEK_TEMP_LOG_PATH"

	EnvCPUProfile = "FARSEEK_CPU_PROFILE"
)
...
func realMain() int {
	...
	if err != nil {
		// openTelemetryInit can only fail if Farseek was run with an
		...
		Ui.Error(fmt.Sprintf("Unset environment variable %s if you don't intend to collect telemetry from Farseek.", tracing.OTELExporterEnvVar))
	...
	ctx, span := tracing.Tracer().Start(ctx, "farseek")
	defer span.End()
	...
	log.Printf("[INFO] Farseek version: %s %s", Version, VersionPrerelease)
	...
	if fips140.Enabled() {
		log.Printf("[WARN] Go runtime FIPS 140-3 mode is enabled; Farseek is not supported in this configuration, which may cause undesirable behavior")
	}
	log.Printf("[INFO] CLI args: %#v", os.Args)
	if experimentsAreAllowed() {
		log.Printf("[INFO] This build of Farseek allows using experimental features")
	}
	...
	if len(diags) > 0 {
		...
		if diags.HasErrors() {
			Ui.Error("As a result of the above problems, Farseek may not behave as intended.\n\n")
			// We continue to run anyway, since Farseek has reasonable defaults.
		}
	}
	...
	// Farseek's behalf using this environment variable. This is used
	...
	// The arguments can begin with a -chdir option to ask Farseek to switch
	...
	if len(diags) > 0 {
		...
		if diags.HasErrors() {
			Ui.Error("As a result of the above problems, Farseek's provider installer may not behave as intended.\n\n")
	...
	if cmd := cliRunner.Subcommand(); cmd != "" && !autoComplete {
		...
		if _, exists := commands[cmd]; !exists {
			...
			fmt.Fprintf(os.Stderr, "Farseek has no command named %q.%s\n\nTo see all of Farseek's top-level commands, run:\n  farseek -help\n\n", cmd, suggestion)
			return 1
		}
	}
	...
	// We might generate some additional log lines if Farseek relied on any
